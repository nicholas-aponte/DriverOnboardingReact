...
import reactTableStyle from "./ReactTableStyles";
...
const useStyles = makeStyles(reactTableStyle);
...
function Table({ columns, data }, props) {
  const [offset, setoffset] = useState(879.5)
  ...
  const [appState, appDisp] = useAppStore();
  //Used as state and updated with the use effect whenever the screen width changes
  const { height, width } = useWindowDimensions();
  const [headStyle, setheadStyle] = React.useState({
    position: 'fixed',
    top: ((window.innerHeight - offset)/2) + "px",
    width: '33%',
    background: 'white'
  })
  ...
  React.useEffect(()=>{
    ...
    if(window.innerWidth>770){
      ...
      setheadStyle({    
        position: 'fixed',
        top: ((window.innerHeight - props.modalYOffset)/2) + "px",
  ...
  useEffect ( () => {
      ...
      if(childRef.current){
          settableHeight(childRef.current.offsetHeight);
          settableWidth(childRef.current.offsetWidth);
      }      
  }, );
  
  let extraTableMargin = {
  ...
  return (
    <>
      <div className="ReactTable -striped -highlight" style={extraTableMargin} ref={parentRef} >

        <table {...getTableProps()} className="rt-table" ref={childRef}>
          <thead className="rt-thead -header" style={headStyle}>
        ...
        <div className="-pagination" style={moving}>
